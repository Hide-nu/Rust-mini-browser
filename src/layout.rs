...
impl<'a> LayoutBox<'a>{
  ...
  fn layout(&mut self, containing_block: Dimentions){
    match self.box_type{
  ...
  fn layout_block(&mut self, containing_block: Dimentions){
  ...
  fn calculate_block_width(&mut self, containing_block: Dimentions){
    ...
    d.padding.right = border_right.to_px();
    
    d.margin.left = margin_left.to_px();
    d.margin.right = margin_right.to_px();
  }

  fn calculate_block_position(&mut self, containing_block: Dimentions){
    let style = self.get_style_node();
    let d = &mut self.dimensions;
    ...
    d.content.y = containing_block.content.y + containing_block.content.height + d.margin.top + d.padding.top + d.border.top;
  }
...
impl Rect {
  pub fn extended_by(self, edge: EdgeSizes) -> Rect {
    Rect{
      x: self.x - edge.left,
      y: self.y - edge.top,
...
impl Dimentions {
  pub fn padding_box(self) -> Rect {
    self.content.extended_by(self.padding)
  }

  pub fn border_box(self) -> Rect {
    self.padding_box().extended_by(self.border)
  }

  pub fn margin_box(self) -> Rect {
    self.border_box().extended_by(self.margin)
  }
}
...
pub fn layout_tree<'a>(node: &'a StyledNode<'a>, mut containing_block: Dimentions) -> LayoutBox<'a> {
